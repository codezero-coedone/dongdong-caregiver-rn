...
const TERMS_CONTENT = [
    {
        ...
        content: '보험계약자 또는 피보험자는 보험청약 시 드리는 질문 내용에 대하여 알고있는 내용을 반드시 사실대로 알려야 합니다. 만일 고의 또는 중대한 과실로 사실과 다르게 알린 경우, 회사는 보험계약자 또는 피보험자의 의사와 관계없이 계약을 해지하거나 보장을 제한할 수 있습니다.',
    },
    {
        title: '2. 해지 환급금',
        content: '보험계약자 또는 피보험자는 보험청약 시 드리는 질문 내용에 대하여 알고있는 내용을 반드시 사실대로 알려야 합니다. 만일 고의 또는 중대한 과실로 사실과 다르게 알린 경우, 회사는 보험계약자 또는 피보험자의 의사와 관계없이 계약을 해지하거나 보장을 제한할 수 있습니다.',
    ...
    return (
        <SafeAreaView style={styles.container} edges={['top', 'bottom']}>
            ...
            <ScrollView style={styles.content} showsVerticalScrollIndicator={false}>
                ...
                <View style={styles.section}>
                    ...
                    <Text style={styles.insuranceDesc}>
                        전문인배상책임보험은 돌봄 서비스 중 발생할 수 있는 간병인의 배상책임을 부담하는 보험입니다.
                    </Text>

                    {/* 자동가입 안내 */}
                    <View style={styles.infoBox}>
                        <Text style={styles.infoBoxTitle}>전문인배상책임보험 자동가입이란?</Text>
                        <Text style={styles.infoBoxText}>
                            최초 보험 자동가입 이후에는 별도의 본인인증 절차 없이 보험 자동가입 동의만으로 일감 지원 및 매칭된 일감을 시작할 수 있습니다.
                        </Text>
                        <Text style={styles.infoBoxSubtitle}>전문인배상책임보험 자동가입 순서</Text>
                        <Text style={styles.infoBoxList}>1. 본인 인증하기</Text>
                        <Text style={styles.infoBoxList}>2. 가입 내용 확인 및 정보 제공 동의하기</Text>
                ...
                <View style={styles.section}>
                    ...
                    <View style={styles.tableCard}>
                        <View style={styles.tableRow}>
                            ...
                            <View style={styles.tableValue}>
                                <Text style={styles.tableValueText}>{MOCK_INSURANCE_DATA.period.start}</Text>
                                <Text style={styles.tableValueText}>~ {MOCK_INSURANCE_DATA.period.end}</Text>
                            </View>
                        </View>

                        <View style={styles.tableRow}>
                            <Text style={styles.tableLabel}>소재지</Text>
                            <Text style={styles.tableValueText}>{MOCK_INSURANCE_DATA.location}</Text>
                        </View>

                        <View style={styles.tableRow}>
                            <Text style={styles.tableLabel}>담보사항{'\n'}전문업무{'\n'}보장내용</Text>
                            <View style={styles.tableValue}>
                                <Text style={styles.tableValueText}>{MOCK_INSURANCE_DATA.coverage.type}</Text>
                                <Text style={styles.tableValueText}>{MOCK_INSURANCE_DATA.coverage.work}</Text>
                                <Text style={styles.tableValueText}>1사고당 : {MOCK_INSURANCE_DATA.coverage.perIncident}</Text>
                                <Text style={styles.tableValueText}>1인당 : {MOCK_INSURANCE_DATA.coverage.perYear}</Text>
                                <Text style={styles.tableValueText}>자기부담금 : {MOCK_INSURANCE_DATA.coverage.deductible}</Text>
                            </View>
                        </View>
                    </View>
                    ...
                    <View style={styles.summaryRow}>
                        ...
                        <Text style={styles.summaryValue}>총 {MOCK_INSURANCE_DATA.careDays}일</Text>
                    </View>

                    <View style={styles.premiumRow}>
                        <Text style={styles.premiumLabel}>총 기간 보험료</Text>
                        <Text style={styles.premiumValue}>{MOCK_INSURANCE_DATA.totalPremium}</Text>
                    </View>
                </View>
                ...
                <View style={styles.section}>
                    ...
                    <TouchableOpacity style={styles.allAgreeRow} onPress={handleToggleAll}>
                        <View style={[styles.checkbox, allAgreed && styles.checkboxChecked]}>
                            {allAgreed && <Ionicons name="checkmark" size={14} color="#fff" />}
                        </View>
                        ...
                        >
                            <View style={styles.agreementLeft}>
                                <View style={[
                                    ...
                                    agreements[item.id] && styles.checkboxSmallChecked
                                ]}>
                                    {agreements[item.id] && <View style={styles.checkboxDot} />}
                                </View>
                                <Text style={styles.agreementText}>
                                    {item.label} <Text style={item.required ? styles.required : styles.optional}>
                                        ({item.required ? '필수' : '선택'})
                                    </Text>
                                </Text>
                            </View>
            ...
            <View style={styles.footer}>
                <TouchableOpacity
                    style={[styles.submitButton, !allRequiredAgreed && styles.submitButtonDisabled]}
                    onPress={handleSubmit}
                    disabled={!allRequiredAgreed}
                >
                    <Text style={[styles.submitButtonText, !allRequiredAgreed && styles.submitButtonTextDisabled]}>
                        간병인배상책임보험 자동가입
                    </Text>
                </TouchableOpacity>
            </View>
