...
- **선택 권한은 JIT(Just-in-time)**: 온보딩에서 “권한 팝업 폭탄” 금지. 기능 진입 시점에만 OS 권한 요청(거부/재시도/설정 이동 포함).
...
#### 인터랙션(1줄)
- (예: “전체 탭 → 다음”, “확인 탭 → 안내 종료(무조건 다음), 실제 권한 요청은 기능 진입 시점(JIT)”)
...
- **박제 여부(QA)**: no
- **인터랙션**: “확인 탭 → 안내 종료(무조건 다음). 실제 권한 요청은 기능 진입 시점(JIT)”
